    ...
    for row in range(len(grid)) :
        for col in range(len(grid[row])):
            if playerRow == row and playerCol == col: #special case for the tile containing the player position
                ...
                print(f"{emojis[grid[row][col]]}", end="")
...
def check_finish(grid, player_position):
    ...
    eg. [["S","*","-"],["*","@","*"],["-","-","F"]] would return False
    but [["S","*","-"],["*","*","*"],["-","-","@"]] would return True
    """
...
def display_help():
    ...
    eg. prints 
    Type "go north/south/east/west" to move around.
    Type "show map" to see the full map and your position.
    You can always type "escape" to quit the game.
    """
    doc= []
    ...
    for line in file :
        row = line.strip() #removes newline character
        print(row)
...
def main():
    ...
    while not stop:
        ...
        if check_finish(map,player_position):
            stop = True
